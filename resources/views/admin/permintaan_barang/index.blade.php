...
@section('content')
              ...
              <tbody style="white-space: nowrap">
                ...
                <tr>
                  ...
                  <td class="text-center">
                    <a class="btn btn-success btn-sm" href="{{ route('permintaan_barang_admin.storelaporan', [$item->id]) }}" onclick="return confirm('Konfirmasi Beli Barang?')"><i class="fa-solid fa-check"></i></a>
                    <a class="btn btn-danger btn-sm" href="{{ route('permintaan_barang_admin.delete', [$item->id]) }}" onclick="return confirm('Apa anda yakin ingin menghapusnya?')"><i class="fa-solid fa-trash-can"></i></a></td>
                </tr>
                @endforeach
              </tbody>
              <tfoot>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div id="showModalTambahBarang">
                    <form action="{{ route('permintaan_barang_admin.store') }}" method="post">
                        @csrf
    ...
    <script>
      ...
      $(document).on('change', '#nama_barang', function(){
        var id = this.value;
        $.ajax({
          url: "{{ route('permintaan_barang_admin.getharga') }}",
          type: "POST",
          data: {
            id:id,
            _token: '{{ csrf_token() }}'},
      ...
      $(document).on('input', '#jumlah', function(){
        ...
        $.ajax({
          url: "{{ route('permintaan_barang_admin.changeharga') }}",
          type: "POST",
          data: {
            jumlah:jumlah,
            id:id,
